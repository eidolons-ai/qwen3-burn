...
Tests use `NdArray` backend (CPU) via dev-dependencies and require no model weights. 50 unit tests cover config parsing, streaming types, RmsNorm, RoPE, causal mask, KV cache, MoE routing, sampling, and end-to-end shape verification for both dense and MoE transformer blocks.
...
- Generation: `generate_streaming()` is the core method; `generate()` is a convenience wrapper
  ...
  - Each chunk uses `build_causal_mask(chunk_len, total_seq_len)` with the correct positional offset; KV cache accumulates across chunks
